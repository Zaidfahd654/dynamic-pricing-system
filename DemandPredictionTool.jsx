...
const MONTH_FACTORS = { jan: 0.8, feb: 0.85, mar: 0.95, apr: 1, may: 1.05, jun: 1.15, jul: 1.2, aug: 1.1, sep: 1, oct: 0.95, nov: 1.1, dec: 1.25 }
...
export default function DemandPredictionTool({ products }) {
  ...
  const predict = () => {
    ...
    const predictedDemand = Math.max(
      1,
      Math.round(baseDemand * priceEffect * seasonEffect * monthEffect)
    )
  ...
  return (
    <Card className="p-6">
      ...
      <Button onClick={predict} className="gap-2 w-full sm:w-auto">
      ...
      {result && (
        <div className="mt-6 p-5 rounded-2xl border">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div>
              <p className="text-xs">Predicted Demand</p>
            ...
            <div className="flex-1">
              <p className="text-xs mb-2">Confidence Range</p>
              <div className="flex items-center gap-2">
                <Badge variant="secondary">{result.low} units</Badge>
                <span className="text-xs">to</span>
                <Badge variant="secondary">{result.high} units</Badge>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
            <div>
              Season factor: <span className="font-semibold">{result.seasonEffect}×</span>
            </div>
            <div>
              Month factor: <span className="font-semibold">{result.monthEffect}×</span>
            </div>
          </div>
        </div>
      )}
