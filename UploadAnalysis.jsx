...
function linearRegression(data) {
  ...
  data.forEach((d, i) => { sumX += i; sumY += d.y; sumXY += i * d.y; sumX2 += i * i; });
...
export default function UploadAnalysis({ data, fileName }) {
  ...
  const { chartData, forecastData, pricing } = useMemo(() => {
    ...
    const forecastData = Array.from({ length: 7 }, (_, i) => {
      const predicted = Math.max(1, Math.round(intercept + slope * (n + i) + Math.sin((n + i) / 3) * avgSold * 0.1));
      return { day: `Day ${i + 1}`, dateLabel: format(addDays(lastDate, i + 1), "MMM d"), predicted };
    });
    ...
    return {
      chartData,
      forecastData,
      pricing: { currentPrice, suggestedPrice, predictedDemand, expectedDemand, revenueBefore, revenueAfter, revenueChange, priceChange: suggestedPrice - currentPrice },
    };
  }, [selectedProduct, data]);

  return (
    <div className="space-y-6">
      {/* File info + product selector */}
      ...
      {/* Price + Units charts side by side */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6">
          <h3 className="font-semibold text-foreground mb-1">Price History</h3>
          <p className="text-xs text-muted-foreground mb-4">Price over time from your data</p>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="uploadPriceGrad" x1="0" y1="0" x2="0" y2="1">
                ...
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "12px", fontSize: "12px" }} formatter={(v) => [`₹${v}`, "Price"]} />
                <Area type="monotone" dataKey="price" stroke="hsl(var(--primary))" fill="url(#uploadPriceGrad)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card className="p-6">
          <h3 className="font-semibold text-foreground mb-1">Demand Trend</h3>
          <p className="text-xs text-muted-foreground mb-4">Units sold over time</p>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="uploadDemGrad" x1="0" y1="0" x2="0" y2="1">
                ...
                <XAxis dataKey="dateLabel" tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" />
                <YAxis tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" />
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "12px", fontSize: "12px" }} formatter={(v) => [v, "Units Sold"]} />
                <Area type="monotone" dataKey="units" stroke="hsl(var(--accent))" fill="url(#uploadDemGrad)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-foreground">Demand Forecast — Next 7 Days</h3>
            </div>
              ...
              {forecastData.map((d, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                  <div>
                    ...
                    <span className="text-xs text-muted-foreground ml-2">{d.dateLabel}</span>
                  </div>
          ...
          <Card className="p-0 overflow-hidden">
            <div className="bg-gradient-to-r from-primary/10 to-accent/10 p-5 border-b border-border">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="font-semibold text-foreground">Suggested Price — {selectedProduct}</h3>
              </div>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between rounded-xl bg-muted/50 px-4 py-3 mt-2">
                ...
                <span className={`text-xl font-bold ${parseFloat(pricing.revenueChange) >= 0 ? "text-accent" : "text-destructive"}`}>
                  {parseFloat(pricing.revenueChange) >= 0 ? "+" : ""}{pricing.revenueChange}%
                </span>
              </div>
            </div>
          </Card>
